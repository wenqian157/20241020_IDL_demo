...
class NatNetClient:
    ...
    def __unpack_rigid_body( self, data, major, minor, rb_num):
        ...
        # Send information to any listener. ### get called every frame
...
#Unpack Mocap Data Functions
    ...
    def __process_message( self, data : bytes, print_level=0):
        ...
        if message_id == self.NAT_FRAMEOFDATA :
            ...
            offset += offset_tmp
            #print("MoCap Frame: %d\n"%(mocap_data.prefix_data.frame_number)) ### where frame count is printed
            # get a string version of the data for output
            mocap_data_str=mocap_data.get_as_string()
            if print_level >= 1:
                # send pos info to pos listener    ### where marker pos is extraced
                ### need to find comcpa_data.rigid_body.pos and send to listener
                #self.pos_listener(mocap_data.labeled_marker_data.labeled_marker_list[0].pos[0])
                #self.pos_listener(mocap_data.labeled_marker_data.labeled_marker_list)
                self.pos_listener(mocap_data.rigid_body_data.rigid_body_list)
                #print("%s\n"%mocap_data_str) ### original print
